...
export class MemStorage implements IStorage {
  ...
  private seedData() {
    ...
    sampleCoas.forEach((coa) => {
      const id = randomUUID();
      this.coas.set(coa.batchNumber, { ...coa, id } as Coa);
    });
  }
  ...
  async getCoaByBatchNumber(batchNumber: string): Promise<Coa | undefined> {
    return this.coas.get(batchNumber);
  }

  async createCoa(insertCoa: InsertCoa): Promise<Coa> {
    const id = randomUUID();
    const coa: Coa = { ...insertCoa, id } as Coa;
    this.coas.set(insertCoa.batchNumber, coa);
    return coa;
  }

  async createOrder(insertOrder: InsertOrder): Promise<Order> {
    const id = randomUUID();
    const order: Order = { ...insertOrder, id, status: "pending" } as Order;
