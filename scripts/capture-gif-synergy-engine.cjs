/**
 * Email 3 — Synergy Engine Showcase GIF
 * 3 scenes: Galaxy view → Stack Builder (BPC-157 + TB-500) → Score reveal (95/100)
 * Usage: node scripts/capture-gif-synergy-engine.cjs
 */
...
async function main() {
  ...
  for (let i = 0; i < TOTAL_FRAMES; i++) {
    ...
    if (i % 12 === 0) process.stdout.write(`  frame ${i+1}/${TOTAL_FRAMES}\r`);
  }
  ...
  spawnSync(FFMPEG, [
    ...
    "-vf", `scale=${WIDTH}:${HEIGHT}:flags=lanczos,palettegen=max_colors=128:stats_mode=full`,
    palette,
  ], { stdio: "inherit" });
  ...
  console.log(`\nDone! → ${OUTPUT}  (${kb} KB, ${TOTAL_FRAMES} frames, ${DURATION_S}s @ ${FPS}fps)`);
}
