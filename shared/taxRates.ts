...
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  // Washington DC (200, 202-205) and Virginia (201 is VA - Dulles/Reston area)
  '200': 'DC', '202': 'DC', '203': 'DC', '204': 'DC', '205': 'DC',
  '201': 'VA', // 201xx is Virginia (DC metro area)
  // Maryland (206-219)
...
export function getStateFromZip(zipCode: string): string | null {
  // Clean the ZIP code - only keep digits
  ...
  // Get the first 3 digits (prefix)
  const prefix = cleanZip.substring(0, 3);
  
  return ZIP_PREFIX_TO_STATE[prefix] || null;
}

// Check if ZIP code is valid (has at least 5 digits and maps to a state)
export function isValidZipCode(zipCode: string): boolean {
  const cleanZip = zipCode.replace(/\D/g, '');
  return cleanZip.length >= 5 && getStateFromZip(zipCode) !== null;
}

// Calculate tax from ZIP code
export function calculateTaxFromZip(zipCode: string, subtotal: number): { tax: number; state: string | null; rate: number } {
  const state = getStateFromZip(zipCode);
  if (!state) {
    return { tax: 0, state: null, rate: 0 };
  }
  
  const rate = getTaxRate(state);
  const tax = calculateTax(state, subtotal);
  
  return { tax, state, rate };
}
