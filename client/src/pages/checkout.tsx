      ...
      <>
        ...
        <main className="min-h-screen pt-20 md:pt-28 pb-36 md:pb-16 px-3 sm:px-4 md:px-8 overflow-x-hidden bg-[#1a1a1f]">
          ...
          <div className="max-w-lg mx-auto w-full">
            ...
            {/* ── Compact order summary bar (collapsible) ── */}
            <div className="bg-[#141414] rounded-xl mb-4 overflow-hidden">
              {/* Collapsed header — always visible, click to toggle */}
              <button
                type="button"
                className="w-full flex items-center gap-3 p-3 text-left"
                onClick={() => setSummaryExpanded(prev => !prev)}
                data-testid="button-toggle-order-summary"
              >
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <p className="font-display font-bold text-lg text-[#E7FB10]">
                    ${Math.round(cartTotal)}
                  </p>
                  <ChevronDown className={`h-4 w-4 text-muted-foreground/60 transition-transform duration-200 ${summaryExpanded ? 'rotate-180' : ''}`} />
                </div>
              </button>
                ...
                {summaryExpanded && (
                  ...
                  >
                    <div className="border-t border-white/[0.06] px-3 pb-3 pt-2 space-y-2">
                      ...
                      {cartItems.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-2.5">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-muted-foreground/70">{item.dosage}{item.quantity > 1 ? ` × ${item.quantity}` : ''}</p>
                          </div>
                      ...
                      {/* Totals */}
                      <div className="border-t border-white/[0.06] pt-2 space-y-1.5 text-xs">
                        <div className="flex justify-between text-muted-foreground">
                          <span>Subtotal</span>
                          <span className="text-foreground">${Math.round(cartSubtotal)}</span>
                        </div>
                        <div className="flex justify-between text-muted-foreground">
                          <span>Shipping{shippingMethod === 'express' ? ' (Express)' : ''}</span>
                          <span className={cartShipping === 0 ? 'text-green-400' : 'text-foreground'}>
                        ...
                        {hasValidZip && (
                          <div className="flex justify-between text-muted-foreground">
                            <span>Tax</span>
                            <span className={cartTax === 0 ? 'text-green-400' : 'text-foreground'}>
                        ...
                        <div className="flex justify-between font-semibold pt-1 border-t border-white/[0.06]">
                          <span>Total</span>
                          <span className="text-[#E7FB10]">${Math.round(cartTotal)}</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
