...
- **Checkout RUO Reminder**: Compact popup on checkout page (smaller than main age gate) requiring users to acknowledge Research Use Only terms and confirm 21+ age before proceeding to payment. Uses sessionStorage for per-session acknowledgment. Features two checkboxes (RUO acknowledgment and age confirmation) that must both be checked before the "Continue to Checkout" button is enabled. Dialog cannot be dismissed by clicking outside or pressing Escape.
