...
- **Custom Stack Builder**: Users can create custom research bundles of 2-4 peptides with a 10% discount, featuring real-time synergy visualization (Synergy Ring, Known Stack Detection with UI badges, Smart Recommendations), Body System Heatmap, Shared Pathway Detection, and save/share functionality. Pathway Overlap card highlights converging receptor systems with a one-time entrance animation, a static amber glow, and an inline cue chip in the synergy ring that smooth-scrolls to the card with a one-shot highlight pulse on click.
- **Reconstitution Wizard** (`/reconstitution-wizard`): Guided 4-step flow (Pick peptide → Vial+BAC water → Syringe+target dose+frequency → Result) with animated 2D SVG insulin syringe (`client/src/components/syringe-svg.tsx`), printable Vial Card PDF download (jspdf+qrcode in `client/src/lib/vial-card-pdf.ts`), and shareable URL with sanitized state encoding (`client/src/lib/reconstitution-math.ts`). Lives alongside the existing math-only calculator at `/tools/peptide-reconstitution-calculator`, which cross-links to it. The wizard does not recommend doses — it only does math from user-entered values, with strong RUO disclaimers on page and in PDF.
...
- **Galaxy Explorer** (`/galaxy`): An interactive 3D peptide universe built with Three.js/React Three Fiber. Galaxy radius 60 units with 7 body-system spiral arms (one arm per system: Healing, Metabolic, Growth, Cognitive, Skin, Longevity, Hormonal) using logarithmic spiral math. Features: galactic core glow (3 billboard layers — nucleus/mid/halo), hub star cross-flares for ~12 high-synergy peptides, sci-fi hover HUD with targeting brackets + top-right telemetry panel, warp flash banner on double-click/search-warp, font-mono HUD typography, font-serif peptide name in side panel, ambient dust stars (2000), filter bar, side-panel detail, cinematic entry, and bottom-left controls. Data in `client/src/data/peptide-pathways.ts`; layout in `client/src/lib/galaxy-layout.ts`; galaxy components in `client/src/components/galaxy/`.
