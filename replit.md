...
- **Galaxy Explorer** (`/galaxy`): An interactive 3D peptide universe built with Three.js/React Three Fiber. Galaxy radius 60 units with 7 body-system clusters (Healing, Metabolic, Growth, Cognitive, Skin, Longevity, Hormonal) arranged in a flat galactic disc. 100 peptides rendered as stars with synergy edges between stacked compounds. Features ambient dust stars (2000 background particles), a filter bar, warp-to search, side-panel detail view, cinematic entry sequence, and bottom-left controls for mute / rotation pause / rotation speed slider. Data lives in `client/src/data/peptide-pathways.ts` and `client/src/data/body-systems.ts`; layout in `client/src/lib/galaxy-layout.ts`.
