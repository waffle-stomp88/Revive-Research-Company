...
- **Dynamic Stack/Bundle Pricing**: Stack and bundle prices are computed dynamically from the database via `/api/products-with-stock`. The utility `client/src/lib/stack-pricing.ts` defines `STACK_COMPONENTS` and `BUNDLE_COMPONENTS` maps (slug + dosage per product per stack/bundle), `buildPriceLookup()` to create a `Map<slug|dosage, price>`, and `calculateStackPricing()` which computes `sum(component prices) × 0.90` (10% discount). No hardcoded prices exist in stack/bundle data — prices update automatically when DB product prices change. Purchase buttons are disabled until pricing loads.
